          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
            {pendingVotes.length > 0 && (
              <button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-md transition-all active:scale-95"
              >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium rounded-xl border border-slate-700 transition-colors flex-shrink-0"
              title="Excel / CSV"
            >
        ...
        <div className="block md:hidden divide-y divide-slate-800/80">
          {paginatedVotes.length === 0 ? (
            <div className="py-10 text-center text-slate-500 text-xs px-4">
              <Vote className="w-7 h-7 mx-auto mb-2 text-slate-600 opacity-50" />
              Tanlangan sana va filter bo'yicha ovozlar topilmadi.
            </div>
          ) : (
            paginatedVotes.map((vote) => (
              <div key={vote.id} className="p-3.5 space-y-2.5 hover:bg-slate-800/30 transition-colors">
                {/* Header */}
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="font-mono font-bold text-white text-xs">+{vote.phone}</span>
                      <span className="text-[10px] text-slate-500 ml-1 font-mono">#{vote.id}</span>
                    </div>
                  </div>
                ...
                {/* Details */}
                <div className="grid grid-cols-2 gap-1.5 text-xs bg-slate-950/60 p-2 rounded-xl border border-slate-800/80">
                  <div>
                    <span className="text-[9px] text-slate-500 block">Foydalanuvchi:</span>
                  ...
                  <div>
                    <span className="text-[9px] text-slate-500 block">Mukofot:</span>
                    <span className="font-bold text-emerald-400 text-xs">+{formatSum(vote.rewardAmount || 30000)} so'm</span>
                  </div>
                ...
                {vote.status !== 'VERIFIED' && (
                  <button
                    ...
                    className="w-full py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-sm transition-colors flex items-center justify-center gap-1.5"
                  >
              ...
              {paginatedVotes.length === 0 ? (
                ...
                paginatedVotes.map((vote) => (
                  <tr key={vote.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="p-3.5 font-mono text-slate-500">#{vote.id}</td>
                    ...
                    <td className="p-3.5 text-right">
                      {vote.status !== 'VERIFIED' ? (
                        <button
                          onClick={() => onApproveVote(vote.id)}
                          className="px-3 py-1 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold rounded-lg shadow-sm transition-colors"
                        >
                          Tasdiqlash
                        </button>
                      ) : (
