        ...
        <div className="block md:hidden divide-y divide-slate-800/80">
          {paginatedWithdrawals.length === 0 ? (
            <div className="py-10 text-center text-slate-500 text-xs px-4">
              <Wallet className="w-7 h-7 mx-auto mb-2 text-slate-600 opacity-50" />
            ...
            paginatedWithdrawals.map((w) => {
              ...
              return (
                <div key={w.id} className="p-3.5 space-y-2.5 hover:bg-slate-800/30 transition-colors">
                  {/* Card Header: Amount & Status */}
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <span className="text-sm font-black text-rose-400">{formatSum(w.amount)} so'm</span>
                      <span className="text-[10px] text-slate-500 ml-1 font-mono">#{w.id}</span>
                    </div>
                  ...
                  <div className="space-y-1.5 bg-slate-950/60 p-2.5 rounded-xl border border-slate-800/80 text-xs">
                    ...
                    <div className="flex items-center justify-between gap-2 pb-1 border-b border-slate-800/60">
                      <div className="flex items-center gap-1.5">
                        <span className="px-1.5 py-0.5 rounded bg-slate-800 border border-slate-700 text-[9px] font-bold text-slate-300 uppercase">
                          {w.paymentMethod}
                        </span>
                      ...
                      <button
                        ...
                        className="p-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 transition-colors flex items-center gap-1 text-[10px]"
                    ...
                    <div className="grid grid-cols-2 gap-1.5 pt-0.5 text-[10px]">
                      <div>
                        <span className="text-slate-500 block">Mijoz:</span>
                        <span className="font-medium text-slate-200 truncate block text-xs">
                          {w.user?.firstName || 'Foydalanuvchi'}
                        </span>
                        {w.user?.phone && (
                          <span className="text-slate-400 font-mono">+{w.user.phone}</span>
                        )}
                      </div>

                      <div>
                        <span className="text-slate-500 block">Karta Egasi:</span>
                        <span className="font-medium text-slate-300 truncate block text-xs">
                          {w.cardHolder || '-'}
                        </span>
                      </div>
                    </div>
                  ...
                  {w.status === 'PENDING' && (
                    <div className="flex items-center gap-2 pt-0.5">
                      <button
                        ...
                        className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl shadow-sm transition-colors flex items-center justify-center gap-1.5"
                      >
                      ...
                      <button
                        ...
                        className="px-3 py-2 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 rounded-xl border border-rose-500/20 transition-colors flex items-center justify-center"
                        title="Rad etish"
                      >
                ...
                paginatedWithdrawals.map((w) => {
                  ...
                  return (
                    <tr key={w.id} className="hover:bg-slate-800/30 transition-colors">
                      <td className="p-3.5 font-mono text-slate-500">#{w.id}</td>
                      ...
                      <td className="p-3.5">
                        <div className="space-y-1">
                          ...
                          {w.receiptUrl && (
                            <button
                              onClick={() => setViewingReceipt(w)}
                              className="flex items-center gap-1 text-[10px] text-indigo-400 hover:text-indigo-300 font-medium block"
                            >
                              <FileCheck className="w-3 h-3" />
                              Chekni ko'rish
                            </button>
                      ...
                      <td className="p-3.5 text-right">
                        {w.status === 'PENDING' ? (
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              onClick={() => onOpenApproveModal(w)}
                              className="px-3 py-1 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-lg shadow-sm transition-colors"
                            >
                              Chek Yuklash
                            </button>
                            ...
                            <button
                              onClick={() => onRejectWithdrawal(w.id)}
                              className="p-1 text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 rounded-lg border border-slate-800 transition-colors"
                              title="Rad etish"
                            >
