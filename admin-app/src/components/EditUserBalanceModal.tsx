      ...
      <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-900/90">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-semibold text-white">Balansni Tahrirlash</h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="p-3 rounded-xl bg-slate-800/60 border border-slate-800 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center text-slate-300 font-semibold text-xs">
                {user.firstName ? user.firstName.charAt(0).toUpperCase() : <User className="w-4 h-4" />}
              </div>
              <div>
                <p className="text-xs font-medium text-white">{user.firstName || `@${user.username}` || 'Foydalanuvchi'}</p>
                <p className="text-[11px] text-slate-400">ID: {user.telegramId} {user.phone ? `• +${user.phone}` : ''}</p>
              </div>
            </div>
            <div className="text-right">
              <span className="text-[10px] text-slate-400 block">Joriy balans:</span>
              <span className="text-xs font-bold text-emerald-400">{formatSum(user.balance)} so'm</span>
            </div>
          </div>

          {/* Operation type toggle */}
          <div>
            <label className="text-xs font-medium text-slate-300 mb-1.5 block">Amal turi</label>
          ...
          {/* Amount input */}
          <div>
            <label className="text-xs font-medium text-slate-300 mb-1.5 block">Summa (so'm)</label>
            <div className="relative">
              <input
                ...
                placeholder="30000"
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors"
                required
              />
              <span className="absolute right-3.5 top-2.5 text-xs text-slate-500">so'm</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-500 rounded-xl shadow-lg shadow-indigo-600/30 disabled:opacity-50 transition-colors"
            >
