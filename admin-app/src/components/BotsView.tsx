      ...
      <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl shadow-lg shadow-indigo-600/30 transition-all active:scale-95"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredBots.map((bot) => {
            ...
            return (
              <div
                key={bot.id}
                className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 hover:border-slate-700 shadow-xl transition-all flex flex-col justify-between"
              >
                <div>
                  {/* Top Avatar & Status */}
                  <div className="flex items-start justify-between gap-3 mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-white tracking-tight">{bot.mahallaName}</h4>
                  ...
                  <div className="p-3.5 rounded-xl bg-slate-800/50 border border-slate-800/80 space-y-2 mb-4">
                    ...
                    <div className="flex justify-between text-[11px] text-slate-400 pt-1">
                      <span>Yig'ildi: <b className="text-white font-medium">{formatSum(bot.currentVotes)}</b> ta</span>
                      <span>Reja: <b className="text-white font-medium">{formatSum(bot.targetVotes)}</b> ta</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs mb-4">
                    <div className="p-2.5 rounded-xl bg-slate-800/30 border border-slate-800">
                      <span className="text-[10px] text-slate-400 block">1 Ovoz mukofoti:</span>
                      <span className="font-bold text-emerald-400">{formatSum(bot.voteReward || 30000)} so'm</span>
                    </div>
                    <div className="p-2.5 rounded-xl bg-slate-800/30 border border-slate-800">
                      <span className="text-[10px] text-slate-400 block">Referal bonusi:</span>
                      <span className="font-bold text-purple-400">+{formatSum(bot.refBonus || 5000)} so'm</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    {bot.status === 'ONLINE' ? (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-rose-400 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 transition-colors"
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 transition-colors"
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 border border-slate-800 transition-colors"
                      title="O'chirish"
                    >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-800/80 text-slate-400 font-semibold border-b border-slate-800 uppercase text-[10px] tracking-wider">
                <tr>
                  <th className="p-4">Bot / Mahalla</th>
                  <th className="p-4">Holati</th>
                  <th className="p-4">Yig'ilgan Ovozlar</th>
                    ...
                    <tr key={bot.id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="space-y-1">
                          <span className="font-bold text-white">{formatSum(bot.currentVotes)} ta ({percentage}%)</span>
                      ...
                      <td className="p-4 font-semibold text-slate-200">{formatSum(bot.targetVotes)} ta</td>
                      <td className="p-4 font-bold text-emerald-400">{formatSum(bot.voteReward || 30000)} so'm</td>
                      <td className="p-4 font-bold text-purple-400">+{formatSum(bot.refBonus || 5000)} so'm</td>
